    ...
    <Fieldset legend={<b>Reset password</b>} className={styles.fieldset}>
        ...
        onSubmit={async (e) => {
          ...
          if (formResetPasswordRef.current) {
            ...
            if (response.status_code === 200) {
              console.log('success!');
              makeToast({
                title: 'Success! Now click the link in your email to set a new password.',
                type: 'success',
                duration: 10000,
              });
            } else {
      ...
      >
        ...
        <p className="text-stone-500 text-[1rem] pt-5 pb-4 text-center">
          You will get an email message with a link to a different page on this site. There, you
          will be able to set a new password.
        </p>
      </form>
    </Fieldset>
  );
