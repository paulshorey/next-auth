'use client';

// import Link from 'next/link';
import VideoItems from '@/src/components/youtube/Items';

export default function YouTubeResultsTemplate({ data }: any) {
  return (
    <div>
      <div className="flex justify-between py-3 px-4 mx-[1.44rem] border-t border-b border-stone-600">
        Options...
        {!!data.prevPageToken && <a href={`?pageToken=${data.prevPageToken}`}>Previous page </a>}
        {!!data.nextPageToken && <a href={`?pageToken=${data.nextPageToken}`}>Next page</a>}
      </div>
      <div className="flex my-6 w-full">
        <div className="pl-[1.44rem] inline-block">
          <VideoItems items={data.items} thumbScale={2} />
        </div>
        <div className="pl-[1.44rem]">Organize your YouTube playlists...</div>
      </div>
    </div>
  );
}
